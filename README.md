# Hyperd Scalable Indexer

A scalable vector indexing system designed to balance **high recall**, **time efficiency**, and **memory optimization**.  
The project explores multiple indexing strategies inspired by **LSH** and **IVFFLAT**, with a focus on rebuildability, parallelization, and disk-based storage.

> The indexer assumes it is built on the full dataset. When new data arrives, the index can be **rebuilt in parallel** while efficiently using disk to minimize RAM usage.

---

## Branches

- **main**  
  Implements a **two-layer indexer (LSH + IVFFLAT)** optimized for large-scale vector search.

- **Kmenas_alone**  
  Implements **IVFFLAT only**, without LSH.  
  Focuses on incremental insertion, dynamic clustering, and memory-efficient reindexing.

---

## Features

- High scalability for large-scale vector datasets
- Efficient speed–recall tradeoff
- Disk-based storage for centroids and clusters
- Parallelizable rebuild process
- Supports approximate nearest neighbor (ANN) search

---

## 1. Two-Layer Indexer (LSH + IVFFLAT)

### Overview

This strategy combines **Locality Sensitive Hashing (LSH)** as a coarse-grained partitioning layer with **K-Means clustering (IVFFLAT-style)** as a fine-grained indexing layer.

The goal is to aggressively reduce the search space using LSH, then apply IVFFLAT inside each bucket to improve recall while maintaining fast query times.

![LSH + IVFFLAT Architecture](./images/img1.png)

---

### Indexing Architecture

#### Layer 1: LSH Bucketing

- Three random projection vectors are used for LSH.
- For each input vector:
  - Cosine similarity is computed against the 3 LSH vectors.
  - A threshold is applied to each similarity score.
  - Each score is converted to either `0` or `1`.
  - The result is a **3-bit binary code**, producing **8 possible buckets**.
- Each vector is assigned to one of the 8 subdomain buckets.

#### Layer 2: IVFFLAT (K-Means per Bucket)

- For each LSH bucket:
  - K-Means clustering is applied.
  - Number of clusters per bucket: **1000**
  - `sklearn.KMeans` is used with `fit()`
- Output per bucket:
  - 1000 cluster centroids
  - Records belonging to each cluster
- Centroids and cluster assignments are persisted to disk for retrieval.

---

### Retrieval Architecture

Given a query vector:

![Retrieval Pipeline](./images/img2.png)

1. Apply the same LSH hashing process.
2. Perform **multi-bucket probing** to search neighboring buckets.
3. Load centroids from disk.
4. Compute cosine similarity between query and centroids.
5. Perform **multi-cluster probing** (`nprobes`).
6. Apply linear search inside selected clusters.
7. Merge and sort results.
8. Return the top **k** nearest vectors.

---

### Experimental Results

**Configuration 1**

- LSH buckets: 8
- LSH nprobes: 6
- IVFFLAT clusters per bucket: 1000
- IVFFLAT nprobes: 150

![Results – Config 1](./images/img3.png)

**Configuration 2**

- LSH buckets: 8
- LSH nprobes: 6
- IVFFLAT clusters per bucket: 1000
- IVFFLAT nprobes: 50

![Results – Config 2](./images/img4.png)

> A result of `-40` means the retrieved ID is 40 positions away from the optimal answer.  
> This is acceptable when operating at scales of **~20 million vectors**.

---

## Notes

- Designed for disk-first, large-scale deployments
- Optimized for rebuildability over real-time mutation
- Suitable for experimentation with ANN tradeoffs
